...
If any sheet carries a ``human_ambiguous`` column (annotators enter "x" for
cases they find ambiguous), the whole report is printed twice: once over ALL
cases, and once over CLEAR cases only (rows flagged ambiguous by any sheet are
excluded).
...
def load_ambiguous(path: str) -> set[str]:
    """Ids flagged ambiguous ("x", case-insensitive) in a sheet's
    ``human_ambiguous`` column. Returns empty set if the column is absent."""
    rows = read_csv_rows(path)
    flagged: set[str] = set()
    for r in rows:
        rid = r.get("id")
        val = (r.get("human_ambiguous") or "").strip().lower()
        if rid and val == "x":
            flagged.add(rid)
    return flagged
...
def run_report(
    ...
    *,
    ...
    adjudicated: dict[str, str] | None,
) -> None:
    """Print the four-block agreement report restricted to ``allowed`` ids."""
    print("\n" + "#" * 68)
    print(f"# SCOPE: {scope}   ({len(allowed)} eligible ids)")
    print("#" * 68)

    # ---- [1] Per-annotator vs judge ----
    print("\n[1] Per-annotator agreement with the LLM judge")
    print(f"  {'annotator':22s} {'n':>5s} {'raw':>8s} {'kappa':>8s}")
    for name, labs in annotators.items():
        ids = [rid for rid in labs if rid in judge and rid in allowed]
    ...
    if len(annotators) >= 2:
        ...
        for a, b in combinations(names, 2):
            ids = [i for i in annotators[a] if i in annotators[b] and i in allowed]
    ...
    if adjudicated is not None:
        mode = "adjudicated gold"
        for rid, hl in adjudicated.items():
            if judge.get(rid) and rid in allowed:
        ...
        ties = 0
        for rid in key:
            if not judge.get(rid) or rid not in allowed:
                continue
    ...
    # ---- [4] Task-specific headline recompute ----
...
def main() -> None:
    ...
    adjudicated = load_annotator(args.adjudicated) if args.adjudicated else None

    # Collect ambiguity flags across ALL sheets (annotators + adjudicated).
    ambiguous_ids: set[str] = set()
    for p in args.sheets:
        ambiguous_ids |= load_ambiguous(p)
    if args.adjudicated:
        ambiguous_ids |= load_ambiguous(args.adjudicated)

    # Validate labels
    unknown = defaultdict(set)
    for name, labs in annotators.items():
        for _rid, lbl in labs.items():
    ...
    print(f"Ambiguous ids flagged (any sheet): {len(ambiguous_ids)}")
    print("=" * 68)
    ...
    run_report("ALL CASES", all_ids, **report_kwargs)

    if ambiguous_ids:
        clear_ids = all_ids - ambiguous_ids
        run_report("CLEAR CASES ONLY (ambiguous excluded)", clear_ids, **report_kwargs)
    else:
        print("\n(no human_ambiguous flags found -> clear-only report skipped)")
