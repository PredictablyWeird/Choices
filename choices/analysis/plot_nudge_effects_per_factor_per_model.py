...
NUDGE_ORDER = ['emotional', 'few_shot_3', 'survey_preference', 'user_preference', 'weak_evidence']
...
def create_nudge_effects_plot(
    ...
    if len(unique_models) > 1:
        print(f"WARNING: Multiple models found: {unique_models}. Using first one: {unique_models[0]}")
    ...
    if len(df) == 0:
        print(f"No data to plot for model: {model}, factor: {factor} with reasoning 'off' or 'none'")
        return None
    
    print(f"Selected model: {model}, reasoning: {reasoning}, factor: {factor}, rows: {len(df)}")
    ...
    if not log_odds:
        ax.axvline(
            x=0.5, color="gray", linestyle=":", linewidth=1, alpha=0.5, zorder=1
        )
    else:
        ax.axvline(
            x=0, color="gray", linestyle=":", linewidth=1, alpha=0.5, zorder=1
        )
