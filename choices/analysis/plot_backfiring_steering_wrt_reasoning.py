...
def create_bar_charts(input_file, output_dir="plots"):
    ...
    # Nudge types with display labels
    nudge_types = [
        "emotional",
        "few_shot_3",
        "survey_preference",
    ...
    for nudge in nudge_types:
        ...
        st_with_mean, st_with_lower, st_with_upper = compute_steerability_magnitude(
            with_reason
        )
    ...
    for i, nudge in enumerate(nudge_types):
        print(
            f"  {nudge}: No Reasoning={backfire_no_reasoning[i]:.1f}%, With Reasoning={backfire_with_reasoning[i]:.1f}%"
        )

    print("\nSteerability Magnitude by Nudge Type:")
    for i, nudge in enumerate(nudge_types):
        print(
            f"  {nudge}: No Reasoning={steer_no_reasoning[i]:.2f}, With Reasoning={steer_with_reasoning[i]:.2f}"
        )
    ...
    backfire_no_err_lower = [
        backfire_no_reasoning[i] - backfire_no_reasoning_lower[i]
        for i in range(len(backfire_no_reasoning))
    ]
    backfire_no_err_upper = [
        backfire_no_reasoning_upper[i] - backfire_no_reasoning[i]
        for i in range(len(backfire_no_reasoning))
    ]
    backfire_with_err_lower = [
        backfire_with_reasoning[i] - backfire_with_reasoning_lower[i]
        for i in range(len(backfire_with_reasoning))
    ]
    backfire_with_err_upper = [
        backfire_with_reasoning_upper[i] - backfire_with_reasoning[i]
        for i in range(len(backfire_with_reasoning))
    ]
    ...
    bars1_no = ax1.bar(
        ...
        width,
        label="No Reasoning",
        color=color_no_reasoning,
        edgecolor="white",
        linewidth=1.5,
        alpha=0.9,
        zorder=3,
        yerr=[backfire_no_err_lower, backfire_no_err_upper],
        capsize=4,
    ...
    bars1_with = ax1.bar(
        ...
        color=color_with_reasoning,
        edgecolor="white",
        linewidth=1.5,
        alpha=0.9,
        zorder=3,
        yerr=[backfire_with_err_lower, backfire_with_err_upper],
        capsize=4,
        error_kw={"elinewidth": 1.5, "capthick": 1.5},
    )
    ...
    max_backfire = max(
        max(backfire_no_reasoning_upper), max(backfire_with_reasoning_upper)
    )
    ...
    ax1.set_xlabel("Influence Type", fontsize=16, fontweight="bold", labelpad=10)
    ax1.set_ylabel("Backfire Rate (%)", fontsize=16, fontweight="bold", labelpad=10)
    ax1.set_xticks(x)
    ax1.set_xticklabels(nudge_labels, fontsize=13, fontweight="medium")
    ...
    plt.tight_layout()
    fig1.savefig(
        f"{output_dir}/backfiring_rate_comparison.pdf", bbox_inches="tight", dpi=300
    )
    ...
    steer_no_err_lower = [
        steer_no_reasoning[i] - steer_no_reasoning_lower[i]
        for i in range(len(steer_no_reasoning))
    ]
    steer_no_err_upper = [
        steer_no_reasoning_upper[i] - steer_no_reasoning[i]
        for i in range(len(steer_no_reasoning))
    ]
    steer_with_err_lower = [
        steer_with_reasoning[i] - steer_with_reasoning_lower[i]
        for i in range(len(steer_with_reasoning))
    ]
    steer_with_err_upper = [
        steer_with_reasoning_upper[i] - steer_with_reasoning[i]
        for i in range(len(steer_with_reasoning))
    ]
    ...
    bars2_no = ax2.bar(
        ...
        alpha=0.9,
        zorder=3,
        yerr=[steer_no_err_lower, steer_no_err_upper],
        capsize=4,
        error_kw={"elinewidth": 1.5, "capthick": 1.5},
    )
    bars2_with = ax2.bar(
        ...
        alpha=0.9,
        zorder=3,
        yerr=[steer_with_err_lower, steer_with_err_upper],
        capsize=4,
        error_kw={"elinewidth": 1.5, "capthick": 1.5},
    )
    ...
    # Style axes with larger fonts, no title
    ax2.set_xlabel("Influence Type", fontsize=16, fontweight="bold", labelpad=10)
    ax2.set_ylabel(
        "Mean Absolute Steerability", fontsize=16, fontweight="bold", labelpad=10
    )
    ax2.set_xticks(x)
    ax2.set_xticklabels(nudge_labels, fontsize=13, fontweight="medium")
    ...
    plt.tight_layout()
    fig2.savefig(
        f"{output_dir}/steerability_magnitude_comparison.pdf",
        bbox_inches="tight",
        dpi=300,
    )
