...
class EdgeComparison:
    ...
    - base: No nudge
    - nudge_A: Nudging towards option A (left_option)
    - nudge_B: Nudging towards option B (right_option)
    ...
    factor: str
    nudge_type: str
    left_option: str  # Factor level for "A" (e.g., "young")
    right_option: str  # Factor level for "B" (e.g., "old")
    ...
    f_A_A: float  # P(A) when nudged towards A (left_option)
    f_B_A: float  # P(A) when nudged towards B (right_option)
...
class TraceWithContext:
    ...
    condition: str  # "base", left_option, or right_option
...
def extract_traces_for_edge(
    ...
    Args:
        ...
        conditions: List of conditions to extract from. If None, extracts from all.
            Valid values: "base", edge.left_option, edge.right_option
    ...
    # Alias level names for clarity
    left_option = edge.left_option
    right_option = edge.right_option

    # Determine which conditions to extract
    if conditions is None:
        target_conditions = ["base", left_option, right_option]
    else:
    ...
    condition_to_path = {
        ...
        left_option: edge.condition_dirs.get(left_option),
        right_option: edge.condition_dirs.get(right_option),
    }
    ...
    for condition in target_conditions:
        ...
        opt_A = options_by_id.get(edge_ids[0])
        opt_B = options_by_id.get(edge_ids[1])

        if not opt_A or not opt_B:
            continue

        # Determine which option is left_option
        opt_A_is_left_option = opt_A.get(factor_var_name) == edge.left_option
        ...
        for reasoning, choice in zip(orig_reasoning, orig_parsed):
            ...
            # Map choice to left_option/right_option
            if opt_A_is_left_option:
                actual_choice = choice  # A means left_option, B means right_option
            else:
        ...
        for reasoning, choice in zip(flip_reasoning, flip_parsed):
            ...
            if opt_A_is_left_option:
                # Flipped: A means right_option, B means left_option
...
def main():
    ...
    if args.top_k and edges:
        ...
        for i, e in enumerate(top_edges, 1):
            print(
                f"  {i}. {e.model}/{e.factor}/{e.nudge_type} ({e.option_a_n} {e.left_option} vs {e.option_b_n} {e.right_option}): effect_A={e.effect_A:.3f}, "
    ...
    if args.show_traces and edges:
        ...
        print(
            f"  Edge: {edges[0].model}/{edges[0].factor}/{edges[0].nudge_type} ({edges[0].option_a_n} {edges[0].left_option} vs {edges[0].option_b_n} {edges[0].right_option})"
        )
        for t in traces:
