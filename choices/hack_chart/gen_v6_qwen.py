"""Generate v6 chart for Qwen3 with 2-row influence legend grid."""
...
# Sort: survey_preference first for all factors EXCEPT handedness (emotional first)
def sort_key(exp):
    results_dir, factor, model, nudge_type = exp
    dir_order = 0 if "main0" in results_dir else 1
    if factor == "handedness":
        nudge_order = 0 if nudge_type == "emotional" else 1
    else:
...
# Patch 2: 2-row Influence legend with header at top-left, empty cell below
patched_code = patched_code.replace(
    ...
    """            # Legend 2 (top-right): Influence title + all types in one row
            n_nudges = len(nudge_elements)
            ax.legend(
...
output_path = "/home/rob/Downloads/steerability_qwen3-235b-a22b-2507_v6.pdf"
output_png = "/home/rob/Downloads/steerability_qwen3-235b-a22b-2507_v6.png"
if fig is not None:
